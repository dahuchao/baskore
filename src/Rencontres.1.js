  ...
  traiter: function (evenement) {
    ...
    evenements[typesEvenement.AJOUT_RENCONTRE] = evenement => {
      ...
      return Rx
        ...
        .map(idCalcule => {
          var rencontre = new Object();
          rencontre.id = idCalcule
          rencontre.date = evenement.date
          rencontre.hote = evenement.hote
          rencontre.visiteur = evenement.visiteur
          // Insertion de la nouvelle rencontre
          bdd
            .collection("rencontres")
            .insert(rencontre, function (err, result) {
              if (err) {
                console.log(`Chargement rencontres en erreur: ${err}.`)
              } else {
                console.log(`Rencontres chargées: ${JSON.stringify(result)}.`)
